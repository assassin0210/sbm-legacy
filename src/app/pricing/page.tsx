...
import { IPricingObject } from '@/shared/api/getPricing'
...
const Pricing = async () => {
  try {
    const response = await instance.get<IPricingObject>(api_links.pricing)
    if (!response?.data) {
      notFound()
    }

    const pricing = Object.keys(response?.data ?? {})
      .map((el) => ({
        ...response?.data[el],
        type: el,
      }))
        ...
        <section className="bg-sbm-neutral-900 pt-6 pb-16">
          <div className="container-v2 mt-28">
            <P38
              ...
              as="h1"
            >
              Start with School Bus Manager risk free for 15 days, then choose
